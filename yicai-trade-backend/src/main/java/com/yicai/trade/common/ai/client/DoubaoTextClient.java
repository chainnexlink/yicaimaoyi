...
@Slf4j
@Component
public class DoubaoTextClient implements AIClient {

    private final AIModelProperties.ModelConfig config;
    private final RestTemplate restTemplate;
    private final ObjectMapper objectMapper;

    public DoubaoTextClient(AIModelProperties properties, RestTemplate restTemplate, ObjectMapper objectMapper) {
        this.config = properties.getModels().get("doubao-text");
    ...
    public AIResponse call(AIRequest request) {
        if (!isEnabled()) {
            return AIResponse.builder()
                    .success(false)
                    .errorMessage("Doubao Text model is disabled")
                    .build();
        }
        ...
        try {
            ...
            log.error("Error calling Doubao Text API", e);
            return AIResponse.builder()
                    .success(false)
